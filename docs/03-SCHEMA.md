...
> **不使用 PG 触发器**：触发器调试不友好；状态机是业务逻辑，不该藏在 SQL 触发器里。
...
> **5 因子锁定（2026-04-29，v3.3.1 已对齐 02-DESIGN）**：02-DESIGN §四 机制七 公式已补 `× f_user_validation` 因子，与本规约 5 因子方案完全一致。`decay_calculations.f_user_validation` 列保留为独立衰减通道（不并入 f_uncertainty）。锁定理由：
> 1. 用户验证是独立信号，不应混入 confidence 通道
> 2. 乘性结构要求 5 条独立通道任一告警都加速遗忘（短板效应）
> 3. 答辩时展示决策生命周期曲线，5 因子分解比 4 因子更清晰
...
| v1.0.1 | 2026-05-04 | §5.1 五因子澄清块改为锁定声明（02-DESIGN v3.3.1 已对齐 5 因子，2026-04-29），移除"⚠️ 需澄清"标记；T-003 启动前清理最后一个开放点 |
