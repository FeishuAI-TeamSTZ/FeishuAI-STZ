...
> **课题**：飞书 AI 校园挑战赛 题二（企业级记忆引擎）· 方向 B
> **版本**：v0.1（框架草稿，2026-05-13）
> **目标读者**：赛事评委 / 项目观察员 / 后续团队接手者
> **目标字数**：~5000 字 + 附录
> **文档地位**：本文是面向评委的**交付物**；内部规约见 [01-CONSTITUTION](./01-CONSTITUTION.md) ~ [06-benchmark-design](./06-benchmark-design.md)
...
"宽定义"（员工产出的所有信息都是记忆）会沦为另一个搜索引擎；"窄定义"（决策原子）使系统能在**关键时刻主动干预**——这正是赛题"产生实际效能"的核心要求。

---

## 2. 切入场景：飞书项目决策一致性（方向 B）（~400 字）

> **填肉指引**：从赛题方向 B 描述 + [02-DESIGN §一.1.3](./02-DESIGN.md) 改写。
...
- ★ **方向 B**：企业核心痛点；技术上需要"演化追踪 + 跨源仲裁"两件没有现成方案的事；答辩篇幅也最适合聚焦
...
| 场景四 | 矛盾更新覆写（赛题测试二）：A → B SUPERSEDES，状态机自动转换 |
...
> 终稿目标：5000 字主体 + 1500 字附录，可导出 PDF 作为决赛交付物
